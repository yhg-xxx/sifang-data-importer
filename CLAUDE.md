...
- **关闭窗口 = 最小化到托盘**：主窗口点 ✕ 不退出程序（首次气泡提示），程序驻留右下角系统托盘；双击托盘图标恢复窗口，右键托盘「退出」才真正退出（任务运行中拒绝退出）；托盘不可用的环境保持关窗即退出
